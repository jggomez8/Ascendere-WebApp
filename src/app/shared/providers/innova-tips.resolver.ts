...
@Injectable()
export class InnovaTipsResolver implements Resolve<InnovaTips> {
  ...
  async resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
    try {
      ...
      if (innovaTipsSnap.empty) return new InnovaTips();
      return new InnovaTips(
        innovaTipsSnap.docs.map(doc => new InnovaTip(Object.assign({ id: doc.id }, doc.data())))
      );
    } catch (error) {
      console.error(error);
      // TODO: add err page
  ...
  ): AngularFirestoreCollection<InnovaTips> {
